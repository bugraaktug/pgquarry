// Adapted from walkrie (Apache-2.0): src/llama_provider.cpp
                                                                  ...
                                                                  int n_embd)
{
    ...
    for (size_t seq_id = 0; seq_id < tokenized.size(); ++seq_id) {
        ...
        const float* embd = llama_get_embeddings_seq(ctx, static_cast<llama_seq_id>(seq_id));
        ...
        if (!embd) {
            ...
            if (last_token_idx >= 0) {
                embd = llama_get_embeddings_ith(ctx, last_token_idx);
            }
        }
